...
provider "proxmox" {
  endpoint    = "https://${var.pve_nodes["inf-pve-01-prd"].network.ip_address}:8006/api2/json" # API endpoint for Proxmox cluster, using first host in cluster.
  api_token   = "${var.pve_api_terraform_user}=${pve_api_terraform_token}" # Pass in variable to avoid hard-coding.
