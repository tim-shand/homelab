# ============================================================================================== #
# Proxmox: Terraform - Networking
# Description: 
# - Terraform configuration for setting up the network bridges on the Proxmox VE hosts. 
# - It defines the Linux bridges for both the management network and the VLANs used by the VMs.
# ============================================================================================== #
...
resource "proxmox_virtual_environment_network_linux_bridge" "pve01_vmbr0" {
  ...
  name          = "vmbr0"
  comment       = "PVE-Hosts" # Description or comment for the bridge.
  address       = "10.0.0.10/24" # Proxmox Host IP address.
  gateway       = "10.0.0.254" # Gateway IP address for the network interface (OPNsense firewall/router).
  autostart     = true
  vlan_aware    = false # Used to carry multiple VLANs.
  ports         = [
    var.pve_host_config_01["pve_nic"] # Network to attach to the bridge, specified by their interface name (eth0).
  ]
}
