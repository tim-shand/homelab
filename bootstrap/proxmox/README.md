...
VM templates are created for the purpose of deploying initial bootstrapped VMs that will provide automated deployments.

- Portable, simple to read, easy to execute.
- Removes application requirements (no need to install Terraform or Ansible locally).
...
- **VM Templates:** Used to deploy Proxmox VMs from templates.
...
- [x] SSH access to a target Proxmox nodes, either singular or in a cluster.
...
4. The Ubuntu VM templates (ID 9000+) will now be listed in the Proxmox interface.
